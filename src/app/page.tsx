...
export default function LoginPage() {
  ...
  function entrarAdmin() {
    // Si se eligió un admin de la lista, su contraseña propia lo identifica
    // (entra con SUS permisos). La contraseña maestra siempre funciona como
    // respaldo y da acceso total (adminId = null).
    const elegido =
      adminSel && adminSel !== "__master__"
        ? admins.find((x) => x.nombre === adminSel)
        : undefined;
    if (elegido && pass === elegido.password) {
      loginAdmin(elegido.id);
      router.push("/admin");
      return;
    }
                    ...
                    {admins.length > 0 && !adminSel ? (
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="grid gap-2">
                          {admins.map((a) => (
                            <button
                              ...
                              className="gs-ripple-host group flex items-center gap-3 rounded-xl border border-emerald-400/10 bg-white/[0.03] p-3 text-left transition-all duration-200 hover:-translate-y-0.5 hover:border-emerald-400/40 hover:bg-emerald-400/[0.07] active:scale-[0.98]"
                            >
                              <span className="flex h-10 w-10 items-center justify-center rounded-full bg-gradient-to-br from-emerald-400 to-teal-600 font-bold text-white">
                        ...
                        <button
                          ...
                          className="text-xs text-emerald-100/50 underline-offset-2 transition-colors hover:text-emerald-200 hover:underline"
                        >
